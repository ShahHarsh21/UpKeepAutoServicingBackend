...
var user={
    ...
    {
         return db.query('insert into user_tbl values(?,?,?,?,?,?,?,?)',[item.user_id,item.email_id,item.password,item.user_name,item.mobile_no,item.address,item.date_of_birth,item.user_type],callback);
    },
    ...
    {
        return db.query('update user_tbl set email_id=?, password=?, user_name=?, mobile_no=?, address=?, date_of_birth=?, user_type=? where user_id=?',[item.email_id,item.password,item.user_name,item.mobile_no,item.address,item.date_of_birth,item.user_type,product_id],callback);
    }
}
