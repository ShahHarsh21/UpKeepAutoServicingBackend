...
var slot={
    ...
    {
         return db.query('insert into slot_tbl (item.vehicle_type,item.vehicle_model,item.service_type,time_period,pickup_time,pickup_address,requirment,drop_address,alloted_employee_id) values(?,?,?,?,?,?,?,?,?)',[item.vehicle_type,item.vehicle_model,item.service_type,item.time_period,item.pickup_time,item.pickup_address,item.requirment,item.drop_address,item.alloted_employee_id],callback);
    },
